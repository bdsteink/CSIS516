...
# Function to load the API key from the .env file or prompt the user if not set
def get_api_key():
    # Load environment variables from the .env file
    load_dotenv()

    # Check if API key exists in the environment variable
    ...
    if not api_key:
        ...
        # Store the API key in the environment for future use
        os.environ["OPENAI_API_KEY"] = api_key

        # Optionally, save the API key to a .env file for future use
...
# Load the API key using the function
api_key = get_api_key()
...
# Declare path for csv
csv_path = os.path.join(script_dir, "wrkoutData.csv")
...
def plot_calories(frame):
    ...
    try:
        ...
        df = pd.read_csv(csv_path, usecols=["Name", "Type", "Duration", "Calories Burned", "Date"], on_bad_lines='warn')
...
class WorkoutApp(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(dash, text="Date:").grid(row = 7, column = 0, sticky = "w", padx=10,pady = 5)
        self.date_entry = DateEntry(dash, width=16,background = 'darkblue', foreground='white', date_pattern='yyyy-mm-dd')
        self.date_entry.grid(row = 7, column = 1, sticky = "w", pady = 5)
        ctk.CTkButton(dash, text="Log Workout", command=self.log_workout).grid(row=8, column=0, padx=10, pady=(10,0))
        ctk.CTkButton(dash, text="Show Calories Burned", command=lambda: plot_calories(self.plot_frame)).grid(row=8, column=1, padx=10, pady=(10,0))
        self.plot_frame = ctk.CTkFrame(dash, corner_radius=8)
        self.plot_frame.grid(row=9, column=0, columnspan=2, sticky="nsew", padx=10, pady=10)
        dash.grid_rowconfigure(9, weight=1)
